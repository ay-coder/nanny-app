                        ...
                        <tbody>
                            @if(count($sitterBookings) > 0)
                                @foreach($sitterBookings as $sitterBooking)
                                    <tr>
                                        <td><a href="{{ route('frontend.user.sitter.booking', ['booking_id' => $sitterBooking->id]) }}"><span class="date"> {{ Carbon\Carbon::parse($sitterBooking->booking_date)->format('d F Y') }}</span></a></td>
